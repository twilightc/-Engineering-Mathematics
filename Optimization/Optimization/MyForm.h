...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->menuStrip1->Size = System::Drawing::Size(46, 24);
...
#pragma endregion
	...
	private: System::Void openFileDialog1_FileOk(System::Object^  sender, System::ComponentModel::CancelEventArgs^  e)
	{
		...
		if (dataManager->LoadEquationData())
		{
			...
			{
				std::vector<std::string> ans = mathop->split_to_format(equations[i]);
				mathop->storeFx(ans);
				ans.clear();
			}
			Output->Text = "-be loaded data-" + Environment::NewLine;
			mathop->print();
		}
	}
	private: System::Void Input_TextChanged(System::Object^  sender, System::EventArgs^  e) {
		if (Input->Text->Length - 1 >= 0 && Input->Text[Input->Text->Length - 1] == '\n') {
			...
			if (userCommand[0] == "newton") {
				if (userCommand[1] == "f:") {
					MarshalString(userCommand[2], t);
					ch = std::stod(t);
				}
				else 
					Output->Text += "-input wrong thing-" + Environment::NewLine;
				
				std::vector<std::string> get;
				bool check = mathop->ch_get_fs(ch,get);
				if (check) {
					if (userCommand[3] == "init1") {
						MarshalString(userCommand[4], t);
						double x = std::stod(t);
						newton->NewtonAlgorism(0,5, x, get);
					}
					else if (userCommand[3] == "init2") {
						MarshalString(userCommand[4], t);
						double x = std::stod(t);
						MarshalString(userCommand[5], t);
						double y = std::stod(t);
						newton->NewtonAlgorism(0, 5, x, y, get);
					}
					else {
						Output->Text += "-input wrong command-" + Environment::NewLine;
					}
				}
				else {
					Output->Text += "-input wrong command-" + Environment::NewLine;
				}
				Output->Text += gcnew String(newton->Outcome().c_str()) 
					         + Environment::NewLine;
				drawpoints(newton->getXs(),get);
			}
			else if (userCommand[0] == "qnewton") {
				if (userCommand[1] == "f:") {
					MarshalString(userCommand[2], t);
					ch = std::stod(t);
				}
				...
				if (check) {
					if (userCommand[3] == "init1") {
						...
						qnewton->QnewtonAlgorism(x,0,get);
					}
					else if (userCommand[3] == "init2") {
						MarshalString(userCommand[4], t);
						double x = std::stod(t);
						MarshalString(userCommand[5], t);
						double y = std::stod(t);
						qnewton->QnewtonAlgorism(x, y, get);
					}
					else {
						Output->Text += "-input wrong command-" + Environment::NewLine;
					}
				...
				Output->Text = "";
				Output->Text += gcnew String(qnewton->getOut().c_str())
							 + Environment::NewLine;
				drawpoints(qnewton->getPs(), get);
			}
			else if (userCommand[0]=="conj") {
				if (userCommand[1] == "f:") {
					MarshalString(userCommand[2], t);
					ch = std::stod(t);
				}
				...
				if (check) {
					if (userCommand[6] == "init1") {
						MarshalString(userCommand[7], t);
						double x = std::stod(t);
						conju->conjugrateAgorism(x,s,e,get);
					}
					else if (userCommand[6] == "init2") {
						MarshalString(userCommand[7], t);
						double x = std::stod(t);
						MarshalString(userCommand[8], t);
						double y = std::stod(t);
						conju->conjugrateAgorism(x,y, s, e, get);
					}
					else {
						Output->Text += "-input wrong command-" + Environment::NewLine;
					}
				...
				Output->Text = "";
				Output->Text += gcnew String(conju->Outcome().c_str())
							 + Environment::NewLine;
				drawpoints(conju->getPs(),get);
			}
		...
		else
		{
			...
			userInput = userCommand[userCommand->Length - 1];
			 ...
			 void drawpoints(std::vector<std::vector<double>> ps,std::vector<std::string> f) {
				 GLenum res = glewInit();
				 if (res != GLEW_OK) {
					 fprintf(stderr, "Error: '%s'\n", glewGetErrorString(res));
				 }
				 ...
				 glPointSize(1);
				 glm::vec3 initialpoint;
				 initialpoint.x = ps[0][0];
				 initialpoint.y = ps[0][1];
				 initialpoint.z = ps[0][2];
				 Points->push_back(initialpoint);
				 PColors->push_back(glm::vec3(0.3921, 0, 0.6078));
				 for (int i = -50; i < 70; i += 2)
				 {
				 ...
				 for (size_t i = 0; i < ps.size() - 1; i++)
				 {
					 ...
					 Lines->push_back(glm::vec3(ps[i][0], ps[i][1],ps[i][2] ));
					 LColors->push_back(glm::vec3(0, 0, 1));


					 Lines->push_back(glm::vec3(ps[i + 1][0], ps[i + 1][1], ps[i + 1][2]));
					 LColors->push_back(glm::vec3(0, 0, 1));
				 }
			 }
